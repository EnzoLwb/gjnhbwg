...
class PaiCheckController extends BaseAdminController
{
	...
	public function pai_list()
	{
		...
		if (request('created_at_from')) {
			$query->where('pai.created_at', '>=', request('created_at_from'));
		}
		if (request('created_at_to')) {
			$query->where('pai.created_at', '<=', date('Y-m-d', strtotime(request('created_at_to') . " +1 day")));
		}
		// 取得列表
	...
	public function pai_comment_list()
	{
		...
		// 筛选发表时间时间
		if (request('created_at_from')) {
			$query->where('pai_comment.created_at', '>=', request('created_at_from'));
		}
		if (request('created_at_to')) {
			$query->where('pai_comment.created_at', '<=', date('Y-m-d', strtotime(request('created_at_to') . " +1 day")));
		}
		// 取得列表
