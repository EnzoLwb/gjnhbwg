...
class AdminUsersController extends BaseAdminController
{
	...
	public function index()
	{
		...
		if (request('created_at_from')) {
			$query->where('admin_users.created_at', '>=', request('created_at_from'));
		}
		if (request('created_at_to')) {
			$query->where('admin_users.created_at', '<=', date('Y-m-d', strtotime(request('created_at_to') . " +1 day")));
		}
		// 取得列表
	...
	public function save()
	{
		...
		if (request('uid')) {
			$this->validate(request(), [
				...
				'nickname' =>[
					'nullable',
					Rule::unique('admin_users', 'nickname')->ignore(request('uid'), 'uid'),
					'max:20'
				],
				'email' => [
					'nullable',
			...
			if(is_email(request('username'))){
				return response()->json([
					'errors'=>[
						'username' => '请不要使用邮箱作为用户名'
					]
				], 422);
			}
			if(is_mobile(request('username'))){
				return response()->json([
					'errors'=>[
						'username' => '请不要使用手机号作为用户名'
					]
				], 422);
			}
			if (!preg_match('/^[\d\w\_]{0,20}$/', request('username'))) {
				// 手动报错
				return response()->json([
					'errors'=>[
						'username' => '用户名只能由数字、字母、下划线组成'
					]
				], 422);
			}
			$this->validate(request(), [
				'username' => 'required|max:20|unique:admin_users',
				'nickname' => 'nullable|max:20|unique:admin_users',
