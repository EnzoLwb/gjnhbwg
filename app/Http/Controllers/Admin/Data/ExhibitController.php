...
class ExhibitController extends BaseAdminController
{
	...
	public function edit($id)
	{
		if (request()->isMethod('post')) {
			...
			if (config('exhibit_config.exhibit.is_lb') && request('is_lb') == 1) {
				...
				$num = Exhibit::where('is_lb', 1)->where('exhibition_id', request('exhibition_id'))->count();
				if ($num >= 5) {
					return $this->error('每个展览最多只能设置' . $lb_num . '个轮播展品');
				}
			}
	...
	public function set_lb($id)
	{
		...
		$num = Exhibit::where('is_lb', 1)->where('exhibition_id', $exhibition_id)->count();
		if ($num >= 5) {
			return $this->error('每个展览最多只能设置5个轮播展品');
		}
