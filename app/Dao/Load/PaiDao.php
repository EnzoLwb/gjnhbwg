...
class PaiDao extends LoadDao
{
	...
	public static function pai_controller($type)
	{
		...
		if ($type == 'check') {
			...
			exec('cd ' . base_path() . ' &&php artisan migrate && php artisan apidoc');
			return true;
			...
			//目录删除
			self::deldir($web_path);
			self::deldir($view_path);
			//删除相关数据库配置
