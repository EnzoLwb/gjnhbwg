...
Route::group([
	...
	Route::group([
		...
		// 上传资源详情页
		Route::get('/file/upload_resource_html/{uploaded_type}/{file_id}/{type}/{now_num}', 'FileController@upload_resource_html')->name('admin.file.file.upload_resource_html');
	});
});
...
Route::group([
	...
	while ($file = readdir($dh)) {
		...
		if ($file != "." && $file != ".." && strstr($file, '.php') !== false&&!preg_match('/[^\x00-\x80]/',$fullpath)) {
			if (!is_dir($fullpath)&&strstr($file, 'web_') !== false) {
