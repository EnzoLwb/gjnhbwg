...
class ExhibitController extends Controller
{
	...
	public function do_like()
	{
		...
		if (empty($is_set)) {
			$r=ExhibitLike::create([
				'uid'=>$uid,'exhibit_id'=>$exhibit_id,'type'=>$type
			]);
			if($type==1){
	...
	public function visit_exhibit(){
		...
		if($uid){
			ExhibitLike::create([
				'uid'=>$uid,'exhibit_id'=>$exhibit_id,'type'=>3
			]);
		}
		//展品收听量+1
		...
		if($uid){
			...
			if(empty($u_ex_info)){
				ExUserVisit::create(['uid'=>$uid,'use_time'=>0,'listen_num'=>1]);
			}
			else{
