...
<style>
	...
	*max width 1600px*
	...
	.ribbon-ing:before{
		/*background: #67A2E4;*/
		background: rgba(103,162,228,0.8);
	}
	.ribbon-end:before{
		/*background: #626262;*/
	...
	.warp-detail .detail-info .info-title{
	    ...
	    overflow: hidden;
	    text-overflow: ellipsis;
	    border-bottom: 1px solid #67A2E4;
	    padding-left: 10px;
	}
...
@section('body')
	<!-- <div class="tab-main">
		<ul>
				<li class="active"><a href="{{route('admin.interaction.question.index')}}">问卷管理</a></li>
		</ul>
	</div> -->
	<div class="js-check-wrap">
		...
		<form class="js-ajax-form" action="" method="post" style="padding: 10px 20px 0px;">
		<div class="user-wrap">
			@foreach($info as $k=>$vo)
				<div class="col-item">
					<div class="col-content" onclick="show_info({{$vo['id']}});" style="cursor: pointer;">
						<!-- 未开始的问卷 -->
						@if($vo->status==2)
						@elseif($vo->status==0)
							<div class="ribbon ribbon-end"><span>未开始</span></div>
						@else
							<div class="ribbon ribbon-ing"><span>进行中</span></div>
						@endif
						<div class="user-message">
							<table>
								...
								<tr>
									...
									@if($vo->status==2)
										<td class="message-never"><i class="fa fa-file-text-o" title="" style="color: #E9E9E9;top: 20%;right: 20%;font-size: 60px;position: absolute;"></i></td>
									@elseif($vo->status==0)
										<td class="message-ing"><i class="fa fa-bookmark-o" title="" style="color: #E9E9E9;top: 20%;right: 20%;font-size: 60px;position: absolute;"></i></td>
									@else
										<td class="message-end"><i class="fa fa-paperclip" title="" style="color: #E9E9E9;top: 20%;right: 20%;font-size: 60px;position: absolute;"></i></td>
									@endif
				...
				<!-- /.col-item -->
			@endforeach
			<div class="col-item" style="width: 223px;min-width: 200px;height:230px;">
...
@section('script')
    ...
    <script type="text/javascript">
    	$(function(){
    		...
    		// $('.m-lang label input[type="radio"]:checked').parent().next().css('border','3px solid #67A2E4').attr('transform', 'scale(1.2)');
...
//   	        		$("#ajax_start_time").html(data.start_time);//开始时间
//   	        		$("#ajax_end_time").html(data.end_time);//结束时间
