...
@section('head')
    <style>
        #position{ width: 500px; height: 245px; overflow: hidden; border-radius: 3px; border: 1px solid #dcdcdc; background-color: #fbf8f1; -webkit-tap-highlight-color: transparent; user-select: none; cursor: default;}
    </style>
@endsection
                    ...
                    <form action="" method="post" class="form-horizontal ajaxForm">
                        ...
                        @foreach(config('exhibit_config.exhibit.imgs') as $k=>$g)
                            <div class="form-group">
                                ...
                                <div class="col-sm-4">
                                    <div class="webuploader-pick" onclick="upload_resource('{{$g['name']}}上传','{{$g['upload_key']}}','{{$g['key']}}',1);" style=" float: left; display: inline-block; width: auto;">点击上传图片</div>
                                </div>
                            </div>
                            <div class="form-group">
                        ...
                        <div class="form-group">
                            <label class="col-sm-2 control-label">所属展览(*)：</label>
                            <div class="col-sm-4">
                            ...
                            <div class="col-sm-4 mapshow">
                                <div name="position" id="position" class="input2" _echarts_instance_="1482218387246">
                                    <div style=" width: 500px; height: 245px; position: relative; overflow: hidden;">
                                        ...
                                        <canvas width="500" height="245" data-zr-dom-id="0" class="zr-element" style=" width: 500px; height: 245px; position: absolute; left: 0px; top: 0px; user-select: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);"></canvas>
                                        <canvas width="500" height="245" data-zr-dom-id="_zrender_hover_" class="zr-element" style=" width: 500px; height: 245px; position: absolute; left: 0px; top: 0px; user-select: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);"></canvas>
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                @foreach(config('language') as $k=>$g)
                                    <div class="layui-tab-item @if($k==1) layui-show @endif">
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-4">
                                                <input type="text" name="exhibit_name_{{$k}}" value="{{$info['language'][$k]['exhibit_name'] or ''}}" class="form-control" @if($k==1) maxlength="25" @else maxlength="500" @endif />
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-4">
                                                <input type="text" name="exhibit_audio_{{$k}}" value="{{$info['language'][$k]['audio'] or ''}}" readonly id="exhibit_audio_{{$k}}" class="form-control" style="width:400px;float: left"/>
...
@section('script')
    ...
    <script>
        ...
        @foreach(config('language') as $k=>$g)
        @foreach(config('exhibit_config.exhibit.content_arr') as $kkk=>$ggg)
        editor{{$ggg['key']}}_{{$k}}= new baidu.editor.ui.Editor({pasteplain:true,initialFrameWidth:950,initialFrameHeight:300,wordCount:false,elementPathEnabled:false,autoHeightEnabled:false,initialStyle:'img{width:20%;}',@if($k==10)iframeCssUrl:'{{cdn('js/plugins/ueditor/themes/vertical_mengyu.css')}}', @endif toolbars: [[
