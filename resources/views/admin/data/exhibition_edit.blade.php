                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-4">
                                    @foreach($exhibition_list as $k=>$g)
                                        <input type="checkbox" @if(!empty($info['near_exhibition'])&&(isset($info['near_exhibition'])&&is_array(json_decode($info['near_exhibition'],true))&&in_array($k,json_decode($info['near_exhibition'],true)))) checked @endif name="near_exhibition[]" value="{{$k}}">{{$g}}
                                    @endforeach
                                </div>
                            </div>
                        @endif
...
@section('script')
    ...
    <script>
        ...
        @foreach(config('language') as $k=>$g)
                editorcontent_{{$k}}= new baidu.editor.ui.Editor({pasteplain:true,initialFrameWidth:initialWidth,initialFrameHeight:initialHeight,wordCount:false,elementPathEnabled:false,autoHeightEnabled:false,initialStyle:'img{width:20%;}',@if($k==10)iframeCssUrl:'{{cdn('js/plugins/ueditor/themes/vertical_mengyu.css')}}', @endif toolbars: [[
